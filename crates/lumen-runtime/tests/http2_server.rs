...
fn server_handles_multiplexed_node_requests() {
    ...
    let child_lines: Vec<_> = String::from_utf8(child.stdout)
        .unwrap()
        .lines()
        .map(str::to_string)
        .collect();
    assert!(
        child_lines.contains(&"/one /one:".to_string()),
        "{child_lines:?}"
    );
    assert!(
        child_lines.contains(&"/two /two:PAYLOAD".to_string()),
        "{child_lines:?}"
    );
    assert_eq!(
