...
fn column(name: &str, kind: u8) -> Vec<u8> {
    let mut out = Vec::new();
    for value in ["def", "database", "users", "users", name, name] {
        out.extend(field(value));
    }
    out.extend([12, 45, 0, 0xff, 0xff, 0xff, 0, kind, 0, 0, 0, 0, 0]);
    out
}
...
fn mysql_adapter_authenticates_escapes_queries_and_decodes_rows() {
    ...
    let peer = std::thread::spawn(move || {
        ...
        greeting.extend(nul("8.0.36-lumen"));
        greeting.extend(1u32.to_le_bytes());
        greeting.extend(b"12345678");
        greeting.push(0);
        greeting.extend(0xffffu16.to_le_bytes());
        greeting.push(45);
        greeting.extend(2u16.to_le_bytes());
        greeting.extend(0xffffu16.to_le_bytes());
        greeting.push(21);
        greeting.extend([0; 10]);
        greeting.extend(b"abcdefghijklm");
        greeting.extend(nul("mysql_native_password"));
        send(&mut stream, 0, &greeting);
        let (sequence, login) = packet(&mut stream);
        assert_eq!(sequence, 1);
        assert!(login.windows(5).any(|bytes| bytes == b"user\0"));
        assert!(login.windows(9).any(|bytes| bytes == b"database\0"));
        send(&mut stream, 2, &[0, 0, 0, 2, 0, 0, 0]);

        let (sequence, query) = packet(&mut stream);
        assert_eq!(sequence, 0);
        assert_eq!(query[0], 3);
        assert_eq!(
            String::from_utf8(query[1..].to_vec()).unwrap(),
            "SELECT 42 AS id, 'O\\'Brien' AS name"
        );
        send(&mut stream, 1, &[2]);
        send(&mut stream, 2, &column("id", 3));
        send(&mut stream, 3, &column("name", 253));
        send(&mut stream, 4, &[0xfe, 0, 0, 2, 0]);
        send(
            &mut stream,
            5,
            &[2, b'4', b'2', 5, b'A', b'l', b'i', b'c', b'e'],
        );
        send(&mut stream, 6, &[0xfe, 0, 0, 2, 0]);
        let (_, query) = packet(&mut stream);
        assert_eq!(
            String::from_utf8(query[1..].to_vec()).unwrap(),
            "UPDATE users SET name = 'Bob' WHERE note = '?' AND id = 42"
        );
    ...
    assert_eq!(
        ...
        "mysql {\"id\":42,\"name\":\"Alice\"} SELECT 1\nchanged 1"
    );
}
