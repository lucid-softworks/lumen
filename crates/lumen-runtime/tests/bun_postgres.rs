...
fn postgres_adapter_uses_extended_protocol_and_decodes_rows() {
    ...
    let peer = std::thread::spawn(move || {
        ...
        assert_eq!(kind, b'p');
        assert_eq!(&password[..password.len() - 1], b"password");
        ...
        loop {
            ...
            if kind == b'P' {
                let start = 1;
                let end = payload[start..].iter().position(|byte| *byte == 0).unwrap() + start;
                ...
                offset += 2;
                for _ in 0..count {
                    let size = i32::from_be_bytes(payload[offset..offset + 4].try_into().unwrap());
                    offset += 4;
                    if size < 0 {
                        params.push(None);
                    } else {
                        params.push(Some(
                            String::from_utf8(payload[offset..offset + size as usize].to_vec())
                                .unwrap(),
                        ));
        ...
        assert_eq!(
            query,
            "SELECT $1::int4 AS id, $2::text AS name, $3::bool AS active"
        );
        assert_eq!(
            params,
            [Some("42".into()), Some("Alice".into()), Some("true".into())]
        );
        send(&mut stream, b'1', &[]);
        send(&mut stream, b'2', &[]);
        let mut description = Vec::new();
        description.extend_from_slice(&3u16.to_be_bytes());
        for (name, oid, size) in [("id", 23u32, 4i16), ("name", 25, -1), ("active", 16, 1)] {
            description.extend(cstr(name));
            ...
            description.extend_from_slice(&size.to_be_bytes());
        ...
        for value in [b"42".as_slice(), b"Alice", b"t"] {
            row.extend_from_slice(&(value.len() as u32).to_be_bytes());
            row.extend_from_slice(value);
        }
        send(&mut stream, b'D', &row);
        send(&mut stream, b'C', b"SELECT 1\0");
        send(&mut stream, b'Z', b"I");
        let (kind, query) = message(&mut stream);
        assert_eq!(kind, b'Q');
        assert_eq!(&query[..query.len() - 1], b"SELECT 1; SELECT 2");
        send(&mut stream, b'C', b"SELECT 1\0");
        send(&mut stream, b'C', b"SELECT 1\0");
        send(&mut stream, b'Z', b"I");
    ...
    assert_eq!(
        ...
        "row {\"id\":42,\"name\":\"Alice\",\"active\":true} SELECT 1\nsimple SELECT 1"
    );
}
...
fn postgres_adapter_authenticates_with_scram_sha_256() {
    ...
    let peer = std::thread::spawn(move || {
        ...
        sasl.extend(b"SCRAM-SHA-256\0\0");
        send(&mut stream, b'R', &sasl);
        let (kind, first) = message(&mut stream);
        assert_eq!(kind, b'p');
        let mechanism_end = first.iter().position(|byte| *byte == 0).unwrap();
        assert_eq!(&first[..mechanism_end], b"SCRAM-SHA-256");
        ...
        continuation.extend(server_first.as_bytes());
        send(&mut stream, b'R', &continuation);
        let (kind, final_message) = message(&mut stream);
        assert_eq!(kind, b'p');
        assert_eq!(
            String::from_utf8(final_message).unwrap(),
            "c=biws,r=fixed-client-nonceserver,p=NiA39R0NbjqSoWjdqDtWyuNcv/Umenu8XmqVHCo1RBo="
        );
        let mut final_auth = 12u32.to_be_bytes().to_vec();
        final_auth.extend(b"v=/znrhjluposUbXiPmX2gG7TFu1ft/gF/hw7xhWY+Orw=");
        send(&mut stream, b'R', &final_auth);
        send(&mut stream, b'R', &0u32.to_be_bytes());
        send(&mut stream, b'Z', b"I");
        loop {
            if message(&mut stream).0 == b'S' {
                break;
            }
        }
        send(&mut stream, b'C', b"SELECT 0\0");
        send(&mut stream, b'Z', b"I");
        assert_eq!(message(&mut stream).0, b'X');
    });
    let mut runtime = Runtime::new();
