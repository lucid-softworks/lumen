//! Server-Sent Events transport (WHATWG HTML §9.2) on `std::net::TcpStream` — a streaming HTTP
...
//! - **`https:`** — rejected with the same TLS-on-std guidance as fetch.
...
struct ConnectResult {
    id: u64,
    outcome: Result<TcpStream, ConnectError>,
}
    ...
    match u.scheme.as_str() {
        ...
        "https" => {
            return Err(ctx.make_error(
                "Error",
                "EventSource: https is not supported yet (TLS cannot be implemented on std \
...
fn open_stream(target: &str, last_event_id: &str) -> Result<TcpStream, ConnectError> {
