...
const POLYMORPHIC_LOCAL_PROPS_SETUP: &str = "\
    const polymorphicLocalObjects=[\
    ...
    function runPolymorphicLocalProps(){\
        let sum=0;\
        for(let i=0;i<100000;i++){\
            let object=polymorphicLocalObjects[i&7];\
            sum+=object.value;\
        }\
        return sum;\
    }";
...
fn main() {
    ...
    engine.eval(POLYMORPHIC_LOCAL_PROPS_SETUP, false).unwrap();
    bench.run("polymorphic-local-props-100k", || {
        black_box(
            engine
                .eval(black_box("runPolymorphicLocalProps()"), false)
                .unwrap(),
        );
    });
    bench.report();
